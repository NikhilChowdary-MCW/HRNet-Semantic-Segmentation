...
std::vector<at::Tensor> backward_cpu(at::Tensor z, at::Tensor dz, at::Tensor var, at::Tensor weight, at::Tensor bias,
  ...
  auto num = count(z);
  at::Tensor dx = (dz - broadcast_to(edz / num, dz) - y * broadcast_to(eydz / num, dz)) * broadcast_to(mul, dz);

  at::Tensor dweight = at::empty(z.type(), {0});
//  std::vector<float>zero = {0};
//  at::Tensor dweight = at::empty(zero);
  at::Tensor dbias = at::empty(z.type(), {0});
  if (affine) {
    dweight = eydz * at::sign(weight);
    dbias = edz;
  }

  std::vector<at::Tensor>temp = {dx, dweight, dbias};
